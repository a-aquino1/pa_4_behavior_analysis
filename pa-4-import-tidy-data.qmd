...
## loaded the tidyverse and janitor package, before commenting them out.
```
...
#| eval: false
library(readr)
youth <- read_csv("data-raw/YRBS2015.csv", skip = 1, na = c("N/A","."), col_types = list(AgeCat = col_number(), 
Grade = col_character(), 
Perception_of_Weight = factor(levels = c("Very underweight", "Slightly underweight", "About the right weight", "Slightly overweight", "Very overweight"))))
  
##moved the data into the data-raw file, later added the skip 1 
```

Describe some of the issues you notice in the data:

> the data is missing some variables
